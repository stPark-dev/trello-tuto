      ...
      <List>
        {["Works", "Insights", "Organisation"].map((text, index) => (
          <Box component={Link} href={`/main/${text.toLowerCase()}`} key={text} sx={{ textDecoration: "none", color: "text.secondary" }}>
            <ListItemButton selected={isActive(`/main/${text.toLowerCase()}`)} onClick={handleDrawerToggle}>
              <ListItemIcon>
                {index === 0 && <TaskAltIcon />}
  ...
  return (
    <Box
      sx={{
        ...
        borderWidth: 1,
        borderBottomColor: "rgb(229 231 235 / 1)",
        ...
        <Box sx={{ display: "flex", height: "4em", justifyContent: "space-between", alignItems: "center" }}>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <Box component={Link} href="/">
              <Image alt="logo" src="/landing/logo_typo.png" width={Math.ceil(1664 / 10)} height={Math.ceil(217 / 10)} />
            </Box>
          </Box>
          <Box
            sx={{
